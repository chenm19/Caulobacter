...
%  kdCtrAClpXP = 0.1*kdCtrAClpXP; %%%%%%% CtrA3 Mutant %%%%%%%
...
dydt(CckAP) = p.kphoCckA*(p.CckAT-y(CckAP))-p.kdephoCckA*(1+p5*p.alpha_cdG*y(cdG)+dephoConstant)*y(CckAP);
...
dydt(cdG)=p.kscdG*(1+p.alpha_PleD*y(PleDP))*p.JicdGcdG^2/(p.JicdGcdG^2+y(cdG)^2)...
    -p.kdcdG*(1+p.alpha_PdeA*y(PdeA))*y(cdG)+p.k3_neg*y(CPLX3)-p.k3_pos*y(cdG)^2*y(CPLX2); %/(y(cdG)+p.JdcdG);
